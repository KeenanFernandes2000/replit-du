...
import { Loader2, Mic, ExternalLink, X } from "lucide-react";
...
interface AIVoiceAgentFormProps {
  trigger?: React.ReactNode;
  className?: string;
}

export function AIVoiceAgentForm({
  trigger,
  className,
}: AIVoiceAgentFormProps) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {isSuccess
              ? "Your AI voice agent has been successfully created. You can now test it!"
              : "Fill out the form below to create your personalized AI voice agent in seconds."}
          </DialogDescription>
        </DialogHeader>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Proposed Agent Name</FormLabel>
                    <FormControl>
                      <Input
